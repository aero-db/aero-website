<template>
  <div v-if="airport">
    <h1 class="text-primary text-3xl font-semibold">
      ...
      <span v-if="airport.iata" class="text-primary-200 text-2xl ml-2 opacity-50">{{ airport.icao }}</span>
    </h1>
    ...
    <div class="mt-4 opacity-70">
      ...
      <div>
        <span class="text-primary-200 mr-2 font-bold" v-if="airport.runways && airport.runways.length > 0">Runways</span>
        <div v-for="runway in airport.runways" :key="runway.name">
          <span class="font-bold">{{ runway.name }}</span> - <span>{{ runway.length }}x{{ runway.width }}ft</span> -
          <span>{{ runway.surface }}</span>
        </div>
      </div>
      <div>
        <span class="text-primary-200 mr-2 font-bold" v-if="airport.frequencies && airport.frequencies.length > 0">Frequencies</span>
